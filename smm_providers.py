    ...
    def add_order(self, service_mapping_id, link, quantity):
        try:
            ...
            if not mapping.service:
                return {"error": f"الخدمة المرتبطة بالتعيين (ID: {mapping.id}) غير موجودة أو تم حذفها"}
            ...
            # إعداد البيانات بشكل صحيح كـ form data
            payload = {
            ...
            response = requests.post(self.provider.api_url, data=payload, headers=headers)
